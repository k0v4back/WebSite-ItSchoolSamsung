...
//echo $key;
  if(isset($_POST['submit'])){
     if(isset($key)){
        ...
        ?>
        <script> alert('Такого ключа не существует!');</script>
        <?php
     }
   }
   ?>
...
<div>
    <div class="login">
  <h2>Введите уникальный ключ</h2>
  <form action="/register" method="POST">
    <input style="border-radius: 5px 5px 5px 5px;" name="key" required="required" placeholder="Уникальный ключ" />
    <!-- <input style="border-radius: 0 0 0 0;" type="email" name="email" required="required" placeholder="Email"/>
    <input style="border-radius: 0 0 5px 5px;" type="password" name="password" required="required" placeholder="Пароль" /> -->
